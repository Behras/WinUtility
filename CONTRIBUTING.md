...
| `src/WinUtility.AppWorker.ps1` | Normal-user fallback for apps that forbid elevation. |
...
2. Confirm that app selection does not install anything and confirmation starts
   ...
   app is skipped. Launch normally and approve startup UAC, then install Spotify
   alongside an app that needs administrator access. Verify Spotify's fallback,
   the other app's elevated install, progress without line flooding, saved output,
...
6. Run the GitHub launcher, including startup UAC cancellation and approval.
   Verify temporary files survive until the elevated menu exits. Check that an
   already elevated shell and `-Preview` never request another UAC approval.
